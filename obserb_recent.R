...
new_stock <- function(stock){
  ...
  command=paste('sh /Users/Zac/stock_conspiracy/wangoo_agentstat.sh',stock,start,end)
...
ob_recent <- function(){
  ...
  if (length(newstock)>0){
    ...
    system('python /Users/Zac/stock_conspiracy/move_files.py recent',ignore.stderr = T)
